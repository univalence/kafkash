...
class ClusterCommand(admin: AdminClient) extends KafkaCliCommand {
  ...
  override val completerNode: TreeCompleter.Node = node(name)

  override def recognize(commandLine: String): Boolean = commandLine == name
  ...
  override def run(commandLine: String): Unit = {
    ...
    val features: FeatureMetadata =
      admin
        .describeFeatures(featuresOptions)
        .featureMetadata()
        .get()
