...
class DeleteCommand(admin: AdminClient, topics: => Seq[String]) extends KafkaCliCommand {

  import scala.jdk.CollectionConverters._

  override val name: String                      = "delete"
  override val completerNode: TreeCompleter.Node = node(name, node("topic", node(new StringsCompleter(topics.asJava))))
